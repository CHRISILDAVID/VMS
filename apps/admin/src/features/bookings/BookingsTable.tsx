    ...
    <div className="flex flex-col h-full bg-slate-50">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <input
              ...
              className="px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 focus:outline-none focus:border-blue-600 focus:bg-white transition-all cursor-pointer"
            />
            {dateFilter && (
              <button
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-sm font-medium text-slate-800">
                {bookings.map((b: any) => {
                  const totalRs = (b.total_amount || 0) / 100;
                  const advanceRs = (b.advance_paid || 0) / 100;
                  const pendingRs = Math.max(0, totalRs - advanceRs);

                  return (
                    <tr key={b.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="py-4 px-4 whitespace-nowrap">
                        <div className="font-bold text-slate-900">{b.booking_number}</div>
                      ...
                      <td className="py-4 px-4">
                        <div className="font-bold text-slate-900">
                          {b.customer?.full_name || b.guest_name || 'Guest User'}
                        </div>
                        <div className="text-xs text-slate-500 font-semibold mt-0.5 flex items-center gap-1">
                          <Phone size={12} />
                          <span>+91 {b.customer?.phone || b.guest_phone || 'N/A'}</span>
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4 whitespace-nowrap">
                        <div className="font-bold text-slate-900">₹{totalRs}</div>
                      ...
                      <td className="py-4 px-4 whitespace-nowrap text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {pendingRs > 0 && b.status !== 'cancelled' && (
                            <button
                              onClick={() => {
                                setShowPaidModal(b);
      ...
      {selectedBooking && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              <div>
                <h3 className="text-lg font-bold text-slate-900">{selectedBooking.booking_number}</h3>
                <p className="text-xs text-slate-500 font-semibold">{selectedBooking.booking_date}</p>
              </div>
              <button onClick={() => setSelectedBooking(null)} className="p-1.5 bg-slate-100 hover:bg-slate-200 rounded-full text-slate-600 cursor-pointer">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="py-6 space-y-4 text-sm">
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-2">
                ...
                <div className="flex justify-between font-semibold">
                  ...
                  <span className="text-slate-900">{selectedBooking.customer?.full_name || selectedBooking.guest_name || 'Guest'}</span>
                </div>
                <div className="flex justify-between font-semibold">
                  <span className="text-slate-500">Phone:</span>
                  <span className="text-slate-900">+91 {selectedBooking.customer?.phone || selectedBooking.guest_phone || 'N/A'}</span>
                </div>
              </div>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-2">
                ...
                <div className="flex justify-between font-semibold">
                  ...
                  <span className="text-slate-900">{selectedBooking.court?.name}</span>
                </div>
                <div className="flex justify-between font-semibold">
                  <span className="text-slate-500">Time:</span>
                  <span className="text-slate-900">{selectedBooking.start_time?.slice(0, 5)} – {selectedBooking.end_time?.slice(0, 5)}</span>
                </div>
                <div className="flex justify-between font-semibold">
                  <span className="text-slate-500">Duration:</span>
                  <span className="text-slate-900">{selectedBooking.duration_minutes / 60} hour(s)</span>
                </div>
              </div>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-2">
                ...
                <div className="flex justify-between font-semibold">
                  ...
                  <span className="text-slate-900">₹{(selectedBooking.total_amount || 0) / 100}</span>
                </div>
                <div className="flex justify-between font-semibold">
                  <span className="text-slate-500">Advance Paid:</span>
                  <span className="text-slate-900">₹{(selectedBooking.advance_paid || 0) / 100}</span>
                </div>
                <div className="flex justify-between font-semibold pt-2 border-t border-slate-200/60">
                  ...
                  <span className={`font-bold ${((selectedBooking.total_amount || 0) - (selectedBooking.advance_paid || 0)) > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                    ₹{Math.max(0, ((selectedBooking.total_amount || 0) - (selectedBooking.advance_paid || 0)) / 100)}
                  </span>
                </div>
              </div>

              {selectedBooking.notes && (
                <div className="bg-amber-50/50 border border-amber-200/60 p-3 rounded-xl text-xs text-amber-900 font-medium">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-lg text-slate-900 focus:outline-none focus:border-blue-600 focus:bg-white"
                />
              </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 focus:outline-none focus:border-red-600 focus:bg-white"
                />
              </div>
