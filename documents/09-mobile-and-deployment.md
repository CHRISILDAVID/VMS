...
## Part E: Player App (ShuttleHub) — Mobile Deployment
...
### 2. Player App-Specific Features

| Feature | Technology | Priority |
|---------|-----------|----------|
| Push notifications | expo-notifications + FCM | MVP |
| Photo capture (profile, ID) | expo-image-picker | MVP |
| GPS location | expo-location | MVP |
| Secure auth storage | expo-secure-store | MVP |
| Offline cache | react-native-mmkv | MVP |
| WhatsApp deep links | react-native Linking | MVP |
| Deep linking | expo-linking + app.json scheme | Phase 2 |
...
### 3. EAS Build Configuration (Player App)
...
### 4. Updated Monorepo Structure

```
VMS/
├── apps/
│   ├── owner/          # React Native (Expo) — Owner mobile app
│   ├── player/         # React Native (Expo) — Player mobile app (ShuttleHub)
│   └── admin/          # React + Vite — Admin web panel
├── packages/
│   └── shared/         # Shared types, services, validators, utils
├── supabase/           # Backend (migrations, edge functions, seed)
├── documents/          # Documentation
├── owner_reference/    # Owner app design reference
├── player_reference/   # Player app design reference
├── package.json        # Workspace root
├── pnpm-workspace.yaml
└── tsconfig.base.json
```

### 5. Updated Environment Architecture

| Environment | Owner App | Player App | Admin Panel | Backend |
|-------------|-----------|------------|-------------|---------|
| Development | Expo Dev Client | Expo Dev Client | localhost:5173 | Supabase (cloud) |
| Preview | EAS internal build | EAS internal build | Vercel preview | Same Supabase |
| Production | EAS production | EAS production | Vercel production | Same Supabase |
